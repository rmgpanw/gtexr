...
- Default values for arguments mirror those for the API e.g. 
...
## Get data for non-significant eQTLs

Some analyses (e.g. Mendelian randomisation) require data for variants which may or may not be significant eQTLs. Use `calculate_eqtls()` with `purrr::map()` to retrieve data for multiple variants

```{r calculate-eqtls}
variants_of_interest <- c("rs6690230",
                          "rs12119111")

variants_of_interest |>
  set_names() |>
  map(\(x) calculate_eqtls(tissueSiteDetailId = "Liver",
